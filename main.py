...
def check_rate_limit(uid, limit=50, window=60):
    now = time.time()
    q = GLOBAL_RL[uid]
...
async def save_history(uid, role, text):
    HISTORY_CACHE[uid].append((role, text[:300]))
...
async def ask_gemini(prompt, retries=None):
...
def build_prompt(user, mode, history, text):
    ...
    parts = [
        ...
        "Chhota, natural, Hinglish reply de. Zyada lamba mat likh.",
...
async def add_queue(uid, chat_id, text, user, priority=5):
    try:
        MESSAGE_QUEUE.put_nowait(
            (priority, next(_QUEUE_COUNTER), (uid, chat_id, text, user))
        )
...
async def worker():
    while not SHUTDOWN_EVENT.is_set():
        try:
            _, _, (uid, chat_id, text, queued_user) = await asyncio.wait_for(
