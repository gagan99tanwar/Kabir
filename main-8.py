...
Optional environment variables:
    ...
    PROACTIVE_CHAT_ID  - group chat_id where Kabir sends random msgs (optional)

Admin commands (only ADMIN_ID):
    .stats                     - uptime, queue, cache info
    .globalstats               - total users, total messages, peak day
    .groupstats                - peak hours, top chatters
    .mode <name>               - switch personality (kabir/roast/coder/gf/rizz/sarcasm/debate)
    .mood <name>               - switch mood (normal/happy/sad/angry/sleepy/roast/excited/jealous/protective)
    .mature on/off             - toggle gaali + flirty mode
    .roastbattle on/off        - toggle full savage roast battle mode
    .profile                   - your full profile
    .memory                    - all saved facts about you
    .set <field> <val>         - manually set a memory field
    .forget <field>            - clear a memory field
    .rel <uid> <status>        - set relationship (neutral/friend/close_friend/best_friend/rival/enemy)
    .tone <uid> <description>  - set per-user tone style
    .ban <uid>                 - ban user from bot
    .unban <uid>               - unban user
    .top                       - top 10 most active users by XP
    .resetxp <uid>             - reset a user's XP and enemy level
    .setmood <uid> <mood>      - set mood for a specific user
    .summary <uid>             - view any user's full profile + summary
    .lastseen <uid>            - check when a user was last active
    .broadcast <msg>           - send message to all known users
    .ignorerate <0-100>        - % chance bot skips replying
    .clearhistory              - wipe your chat history
    .keys                      - Gemini key health
    .ping                      - latency check
...
Open commands (everyone can use):
    ...
    .roastme                   - get roasted by Kabir
    .roast @name               - roast someone in the group
    .debate <topic>            - Kabir argues a side on any topic
"""
